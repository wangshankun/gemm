...
int main()
{
    ...
    sgemm_thread_nn_me(abuff, bbuff, cbuff, m, n, k);
    clock_gettime(CLOCK_MONOTONIC, &finish);
    elapsed = (finish.tv_sec - start.tv_sec);
    elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
    printf("%d threads sgemm_me elapsed time:%f cbuff[%d]:%f\r\n",
		    MAX_CPU_NUMBER_, elapsed, sampling_index, cbuff[sampling_index]);
    sub_pthread_exit();
    ...
    elapsed = (finish.tv_sec - start.tv_sec);
    elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
    printf("single thread native_c elapsed  time:%f cbuff[%d]:%f\r\n",
    ...
    elapsed = (finish.tv_sec - start.tv_sec);
    elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
    printf("4 thread openblas elapsed  time:%f cbuff[%d]:%f\r\n", elapsed, 
		    sampling_index, cbuff[sampling_index]);
