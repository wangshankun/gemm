...
//extern void print_matrix( int m, int n, float *a, int lda );
...
int main()
{
    ...
    memset(cbuff, 0 , m * n * sizeof(float));
    clock_gettime(CLOCK_MONOTONIC, &start);
    MY_MMult_13(m, n, k, abuff, lda, bbuff, ldb, cbuff, ldc);
    clock_gettime(CLOCK_MONOTONIC, &finish);
    elapsed = (finish.tv_sec - start.tv_sec);
    elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
    printf("my_gemm elapsed  time:%f cbuff[%d]:%f\r\n",elapsed, sampling_index, cbuff[sampling_index]);
